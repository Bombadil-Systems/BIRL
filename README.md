...
Most security tools parse files one format at a time. BIRL parses them as *all formats simultaneously*, measures which bytes each parser actually claims, and identifies the ones nobody does. Those unclaimed bytes — the residue — are where polyglots live, where payloads hide, and where detection breaks down.
